...
export class IncomeListComponent implements OnInit {
  ...
  //     if (this.editingIncome.date) {
  //       let dateToSave: Timestamp | null = null;
  //       if (typeof this.editingIncome.date === 'string') {
  //         try {
  //           const [month, day, year] = this.editingIncome.date
  //             .split('/')
  //             .map(Number);
  //           dateToSave = Timestamp.fromDate(new Date(year, month - 1, day));
  ...
  //         dateToSave = Timestamp.fromDate(this.editingIncome.date);
  //         this.editingIncome.date = dateToSave; // Assign the Timestamp back
  //       } else if (this.editingIncome.date instanceof Timestamp) {
  //         dateToSave = this.editingIncome.date; // It's already a Timestamp
  //       }
